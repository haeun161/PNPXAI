      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`${size === "xs" ? "text-[10px] px-2 py-1" : "text-xs px-2.5 py-1"} rounded-md border transition-colors ${
              isActive
...
export default function ResultsPanel({ results, task, rankingMetric: _unused }: Props) {
      ...
      <div className="fixed inset-0 z-50 bg-white overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-3">
            <MetricButtons options={RANKING_OPTIONS} active={activeMetrics} onToggle={toggleMetric} size="sm" />
            <button
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {rankedResults.map((r) => (
              <ResultCard key={r.explainer_name} result={r} task={task} activeMetrics={activeMetrics} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <MetricButtons options={RANKING_OPTIONS} active={activeMetrics} onToggle={toggleMetric} size="xs" />
      </div>
      <div className="flex gap-4 overflow-x-auto pb-2">
        {rankedResults.map((r) => (
          <div key={r.explainer_name} className="flex-shrink-0" style={{ width: "calc((100% - 2rem) / 3)" }}>
            <ResultCard result={r} task={task} activeMetrics={activeMetrics} />
          </div>
        ))}
      </div>
    </div>
